...
def hough_vote_lines(img):
    ...
    # YOUR CODE HERE
    no_of_theta = 181
    theta_array = np.linspace(0, 180, no_of_theta)
    theta_array = np.deg2rad(theta_array)
    d = np.ceil(np.sqrt((len(img) ** 2) + (len(img[0]) ** 2)))
    no_of_rho = int(1 + 2 * d)
    rho_array = np.linspace(-1 * d, d, no_of_rho)
        ...
        for j in range(len(img[0])):
            if img[i, j] != 0:
                for k in range(len(theta_array)):
                    ...
                    index = np.argmin(np.abs(rho_array - rho))
                    A[index, k] += 1

    # END
            
    return A, rho_array, theta_array
...
def hough_vote_circles(img, radius = None):
    ...
    for ind_radius in R:
        ...
        #3.1 Creating a circular mask
            rr, cc = circle_perimeter(edge[0], edge[1], ind_radius, shape=(h, w))
        ...
        #    Center the mask over that point and update the accumulator array
            A[ind_radius - R_min, rr, cc] += (1 / (ind_radius))
